...
static int fb_notifier_callback(struct notifier_block *self, unsigned long event, void *data)
{
	...
	case MSM_DRM_BLANK_UNBLANK:
		if (MSM_DRM_EARLY_EVENT_BLANK == event) {
			// Panel is about to init and may toggle shared reset GPIO.
			// Don't resume yet — wait until panel is fully done (EVENT_BLANK).
		} else if (MSM_DRM_EVENT_BLANK == event) {
			// Display fully initialized. Touch IC may have been corrupted
			// by panel's reset GPIO toggle. Call resume directly (not
			// queue_work) to ensure it runs even with frozen workqueues.
			// Re-arm suspended so the full HW reset always runs — the
			// display may init multiple times during boot and each time
			// the shared GPIO gets toggled, corrupting the IC again.
